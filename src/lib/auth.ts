...
const MASTER_PASSWORD = "9319123410";
...
export async function verifyAppPassword(input: string): Promise<boolean> {
  const stored = localStorage.getItem(STORAGE_PASSWORD_HASH);
...
export function isLoggedIn(): boolean {
  const ts = localStorage.getItem(STORAGE_LOGIN_TS);
  if (!ts) return false;
...
export function startSession(): void {
  localStorage.setItem(STORAGE_LOGIN_TS, String(Date.now()));
}
...
export async function login(password: string): Promise<boolean> {
  ...
  if (!ok) return false;
  // Persist default hash if needed
  if (!localStorage.getItem(STORAGE_PASSWORD_HASH) && password === DEFAULT_APP_PASSWORD) {
